...
tags:
  ...
  - 技术
mathjax: false
pubDatetime: 2019-02-16T15:57:42.000Z
description: >-
  问题产生 在线上主从同步的slave数据库中产生了如下的错误：  使用可以查看从库当前的同步状态,发现I/O状态已经变为NO,尝试使用: 
  之后产生如下的问题:  根据错误说明使用一个空事务来跳过异常,代码如下: 
  然而这样会导致主从的数据可能产生不一致的情况,这是我们想要避免的,并且希望主库可以不锁表导出数据即可进行...
---
